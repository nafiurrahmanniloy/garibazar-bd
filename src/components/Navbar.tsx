    ...
    >
      <div className="max-w-[1280px] mx-auto flex items-center justify-between">
        ...
        <div className={`nav-links flex items-center gap-8 max-md:fixed max-md:top-0 max-md:right-0 max-md:w-[280px] max-md:h-screen max-md:bg-[var(--bg-secondary)] max-md:flex-col max-md:justify-center max-md:gap-6 max-md:p-10 max-md:border-l max-md:border-white/[0.06] max-md:z-[999] max-md:transition-transform max-md:duration-300 ${mobileOpen ? "max-md:translate-x-0" : "max-md:translate-x-full"}`}>
          {["Home", "Browse Cars", "How It Works", "About", "Contact"].map((item) => (
            <a
              key={item}
              href={`#${item.toLowerCase().replace(/\s+/g, "-")}`}
              className="text-sm font-medium text-[var(--text-secondary)] hover:text-white transition-colors relative after:content-[''] after:absolute after:bottom-[-4px] after:left-0 after:w-0 after:h-[2px] after:bg-[var(--accent)] after:transition-all after:duration-300 hover:after:w-full"
              onClick={() => setMobileOpen(false)}
            >
              {item}
            </a>
          ))}
          <button className="px-6 py-2.5 bg-[var(--accent)] hover:bg-[var(--accent-hover)] text-white rounded-lg font-semibold text-sm transition-all hover:-translate-y-0.5 hover:shadow-[0_4px_20px_var(--accent-glow)]">
            গাড়ি বিক্রি করুন
          </button>
        </div>
